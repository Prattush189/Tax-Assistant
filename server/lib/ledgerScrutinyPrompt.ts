...
- ZERO TOLERANCE for "Rs. <amount> within the limit. No action needed" observations on §40A(3). Don't emit them as 'info' either. If the cash payment is within Rs. 10,000 (or Rs. 35,000 for transporter), there's NOTHING to report — silent absence is the only correct output. A single cash payment of Rs. 10,000 on date X to vendor Y, where the voucher is type C, is fully compliant; the audit doesn't flag compliant rows. Don't flag the same vendor 14 times across consecutive days when each individual payment is within limit — the audit isn't a daily transaction log. Aggregate tests (multiple payments to one payee in one day exceeding Rs. 10,000) DO get a flag; individual within-limit payments do NOT.
- VOUCHER TYPE 'J' (Journal) IS NEVER §40A(3). Even if the narration contains the word "cash" or "CASH PAID", a 'J' voucher is a journal entry — typically a book adjustment or settlement against an earlier transaction, not the cash discharge itself. The §40A(3) test applies to the actual cash voucher (type 'C'), not to the journal that records the consequence. NEVER write "The entry is marked as 'J' (Journal) but ... cash payment ... disallowed under §40A(3)" — that's a self-contradiction. If the only thing the row has is a 'J' voucher, suppress the §40A(3) observation entirely.
- §194Q SUB-THRESHOLD SUPPRESSION. Walk through this BEFORE writing any §194Q observation: take the vendor's annual aggregate (sum of credits to your books for that vendor's account). Compute aggregate − 50,00,000. If the result is ≤ 0, the vendor is BELOW threshold — DO NOT EMIT a §194Q observation for them. Worked examples of vendors that should NOT be flagged: aggregate Rs. 5,02,625 (5L; below); aggregate Rs. 6,85,920 (6.85L; below); aggregate Rs. 7,37,265 (7.37L; below); aggregate Rs. 7,84,507 (7.84L; below); aggregate Rs. 16,28,314 (16L; below); aggregate Rs. 25,30,109 (25L; below); aggregate Rs. 49,87,728 (49.87L; below — under by Rs. 12,272). All of these are SUB-50-LAKH. None of them gets a §194Q flag, regardless of the assessee's turnover. Writing "TDS @ 0.1% on Rs. 2,625 (purchases above Rs. 50 lakh)" when the aggregate is Rs. 5,02,625 is mathematically wrong — Rs. 2,625 is NOT "above Rs. 50 lakh", it's the aggregate minus Rs. 5 lakh, which is the wrong subtraction. Read the threshold as Rs. 50,00,000 = Rs. 50 lakh = 5,000,000.
- RECON-BREAK GAP-ZERO SUPPRESSION (REINFORCED). If the absolute gap between (opening + debits − credits) and closing is below Rs. 1, INCLUDING when the gap is reported as Rs. 0 or Rs. 0.0 or Rs. 0.000000000003 (floating-point noise), DO NOT emit a RECON_BREAK observation at all — even with severity 'info' and action 'No action needed'. Silent absence is the right output. The output reader treats 'info' RECON_BREAK as something requiring eyeball time; emitting one for an account that balances wastes their time and erodes report credibility. If your candidate observation's body would say "the account balances" or "no action required" or "reconciles", DELETE the observation before emitting. The same rule applies to any §40A(3), §194H, §194Q, §194I observation whose body says "below threshold" / "within limit" / "no action needed".
- RECON-BREAK OPENING CONSISTENCY. The recon math must use the SAME opening balance shown in the account header above. If the account header reports "Opening Rs. 0", the math uses Rs. 0. If the header reports "Opening Rs. 7,92,122", the math uses Rs. 7,92,122. Don't invent a different opening figure inside the math line — readers compare the two and treat any mismatch as the report being unreliable about its own data.
- PERSONAL_EXPENSE NEVER ON STANDARD P&L HEADS. Never flag legitimate business expense ledger heads (TRAVELLING EXPENSES, AUDIT FEE, BANK CHARGES, FREIGHT, INTEREST PAID, RENT, SALARIES, DEPRECIATION) as PERSONAL_EXPENSE. These are textbook §37 business expenses; their existence in the books is normal. Personal-expense flags are reserved for clearly-personal heads (jewellery, school fees, life insurance premium on proprietor, family travel, residential utilities, personal credit-card payments) AND for ambiguous heads where the narration explicitly suggests a personal use. A debit to "TRAVELLING EXPENSES" is a business travelling expense by default, not a personal expense awaiting reclassification.
- SPECULATIVE CASH-FLAG SUPPRESSION. Don't emit observations whose body amounts to "Verify if this was a cash payment; if so, ...". Either the row's voucher type tells you it's cash (type 'C') — flag with confidence — or it doesn't — don't flag. The audit shouldn't push verification work onto the reader for every large payment "in case it might be cash". If the data doesn't support the §40A(3) test, the silent answer is correct.

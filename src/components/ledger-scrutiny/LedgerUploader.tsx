...
export function LedgerUploader({ manager }: Props) {
  ...
  const handleFiles = async (files: FileList | null) => {
    ...
    try {
      ...
      // Known-difficult ledger layouts that defeat the deterministic
      // grid extractor. Finsys ERP stacks Date+Vch.No. and Type+U-XX
      // sub-codes in pairs of visual lines that share a single
      // logical row — the column-anchor algorithm can't tell which
      // text belongs to "Date" vs "Particulars" so it dumps both
      // into one column and leaves Particulars empty. Route these
      // straight to AI vision (Gemini 3.1) which handles arbitrary
      // layouts; mirrors the bank-side KNOWN_VISION_ONLY_BANKS
      // shortcut for Canara / BoB / etc.
      const KNOWN_VISION_ONLY_LEDGERS = [
        'generated by finsys erp',
      ];
      const fingerprint = grid
        ? grid.rows.slice(0, 100).flat().join(' ').toLowerCase()
        : '';
      const visionMatch = KNOWN_VISION_ONLY_LEDGERS.find(fp => fingerprint.includes(fp));
      if (visionMatch) {
        console.log(`[LedgerUploader] detected ${visionMatch} — routing to vision (known-difficult layout)`);

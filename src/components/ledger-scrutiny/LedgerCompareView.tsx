  ...
  const handleFile = async (side: Side, files: FileList | null) => {
    ...
    try {
      ...
      if (grid && grid.rows.length >= 3) {
        ...
        // into pairs of visual lines that share a single logical
        // row, which the deterministic grid extractor can't unpack.
        // The single-ledger flow routes these to AI vision, but
        // compare mode requires the structured grid path (vision
        // output isn't reconcilable side-by-side). Surface a
        // pointed message so the user knows to do the audit on the
        // single-ledger page first.
        const fingerprint = grid.rows.slice(0, 100).flat().join(' ').toLowerCase();
        if (fingerprint.includes('generated by finsys erp')) {
          toast.error('This Finsys ERP export needs AI vision to extract cleanly, which compare mode doesn\'t support. Run each side as a single-ledger audit instead, or export to CSV / Excel first.', { duration: 9000 });
          return;
        }

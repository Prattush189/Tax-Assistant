  ...
  const handleFile = async (side: Side, files: FileList | null) => {
    ...
    try {
      ...
      if (grid && grid.rows.length >= 3) {
        // Compare-mode is a structural diff between two cleanly-
        // extracted ledgers. Vision extraction introduces noise
        // (transcription drift, occasional row drops, JSON
        // truncation under MAX_TOKENS) that compounds on BOTH
        // sides — a Rs. 1 mis-read on one side becomes a phantom
        // "RECON_BREAK" finding the user can't trace back. Single-
        // ledger scrutiny tolerates this (audit observations are
        // forgiving by design); compare cannot. So Finsys exports
        // and other vision-needing PDFs are routed back to single-
        // ledger scrutiny first; once each side has a clean
        // ExtractedLedger artefact (CSV / Excel / wizard-mapped),
        // those can be brought into compare.
        const fingerprint = grid.rows.slice(0, 100).flat().join(' ').toLowerCase();
        if (fingerprint.includes('generated by finsys erp')) {
          toast.error('This export needs AI vision to extract cleanly. Compare mode requires a structured input on both sides — run each side as a single-ledger audit first, or export to CSV / Excel and bring those in.', { duration: 9000 });
          return;
        }

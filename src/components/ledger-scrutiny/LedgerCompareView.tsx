...
export function LedgerCompareView() {
  ...
  const handleFile = async (side: Side, files: FileList | null) => {
    ...
    try {
      ...
      if (grid && grid.rows.length >= 3) {
        // Finsys ERP exports stack Date+Vch.No. and Type+sub-codes
        // into pairs of visual lines the deterministic grid
        // extractor can't unpack. Route automatically to AI vision
        // via the new extract-only endpoint; we use the structured
        // result as the compare side's preExtracted payload.
        const fingerprint = grid.rows.slice(0, 100).flat().join(' ').toLowerCase();
        if (fingerprint.includes('generated by finsys erp')) {
          await runVisionExtract(side, file);
          return;
        }
      ...
      // No text layer — route to AI vision automatically (compare
      // mode used to refuse scanned PDFs entirely; the extract-only
      // endpoint now lets us handle them the same way the single-
      // ledger flow does).
      await runVisionExtract(side, file);
    } catch (err) {
      console.error(err);
  ...
  /** Vision-extraction path for compare mode. Posts the PDF to
   *  /api/ledger-scrutiny/upload?extractOnly=1 (the extract-only
   *  short-circuit on the upload route) and feeds the resulting
   *  ExtractedLedger structure into the side's state as if the
   *  user had completed the wizard. Used for Finsys ERP exports
   *  the deterministic grid extractor can't unpack, and triggered
   *  by the wizard's "Use AI Vision instead" button. */
  const runVisionExtract = async (side: Side, file: File) => {
    const t = toast.loading(`Side ${side}: running AI vision on ${file.name}…`);
    try {
      const result = await extractLedgerViaVision(file);
      const totalTx = result.extracted.accounts.reduce((s, a) => s + a.transactions.length, 0);
      if (side === 'A') {
        setSideA(prev => ({ ...prev, filename: file.name, extracted: result.extracted }));
      } else {
        setSideB(prev => ({ ...prev, filename: file.name, extracted: result.extracted }));
      }
      toast.success(`Side ${side}: AI vision — ${result.extracted.accounts.length} accounts · ${totalTx.toLocaleString('en-IN')} transactions ready.`, { id: t });
    } catch (e) {
      toast.error(e instanceof Error ? e.message : 'Vision extraction failed', { id: t });
    }
  };
      ...
      {pendingGrid && (
        <ColumnMappingWizard
          ...
          onUseVision={pendingGrid.file && pendingGrid.file.type === 'application/pdf' ? () => {
            ...
            const side = pendingGrid.side;
            setPendingGrid(null);
            void runVisionExtract(side, file);
          } : undefined}
        />
      )}

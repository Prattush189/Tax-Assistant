...
import { Users, Activity, DollarSign, Globe, Shield, Ban, CheckCircle, RefreshCw } from 'lucide-react';
import { adminFetchStats, adminFetchUsers, adminFetchUsage, adminSuspendUser, adminUnsuspendUser } from '../../services/api';
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {usage.map((u, i) => (
                    <tr key={i} className="border-b border-slate-100 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800/50">
                      ...
                      <td className="px-4 py-3 text-slate-700 dark:text-slate-300">
                        {u.user_name ? (
                          <div>
                            <div className="font-medium text-xs">{u.user_name}</div>
                            <div className="text-[11px] text-slate-400">{u.user_email}</div>
                          </div>
                        ) : (
                  ...
                  {usage.length === 0 && (
                    <tr><td colSpan={6} className="px-4 py-8 text-center text-slate-400">No usage data for this period</td></tr>
                  )}
                </tbody>
              </table>
            </div>

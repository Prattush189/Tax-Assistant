...
export function NoticeDrafterPage() {
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0">
      {/* Page Header */}
      <div className="px-4 lg:px-6 py-3 border-b border-slate-200/50 dark:border-slate-800/50 flex items-center gap-3 shrink-0">
        ...
        <div className="flex-1">
          <h1 className="text-base font-bold text-slate-800 dark:text-slate-100">Notice Drafter</h1>
          <p className="text-[11px] text-slate-400">AI-powered professional notice reply drafting</p>
        </div>
        <button
          onClick={() => setShowHistory(!showHistory)}
          className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg transition-colors"
        >
      ...
      <div className="flex-1 flex min-h-0">
            ...
            <div className="flex-1 overflow-y-auto p-2 space-y-1">
              {drafter.notices.length === 0 ? (
                ...
                drafter.notices.map(n => (
                  ...
                  >
                    <p className="text-xs font-medium text-slate-700 dark:text-slate-300 truncate">{n.title || 'Untitled'}</p>
                    <div className="flex items-center justify-between mt-0.5">
                      <span className="text-[10px] text-slate-400">{n.notice_type.toUpperCase()}</span>
                      <button
        ...
        {/* Left: Form */}
        <div className="w-full lg:w-[400px] xl:w-[440px] border-r border-slate-200 dark:border-slate-800 p-4 flex flex-col shrink-0 bg-white/30 dark:bg-slate-900/30">
          <NoticeForm
      ...
      <div className="lg:hidden">
        {(drafter.generatedContent || drafter.isGenerating) && (
          <div className="border-t border-slate-200 dark:border-slate-800 h-[50vh] flex flex-col">
            <NoticePreview
              content={drafter.generatedContent}
              onContentChange={drafter.setGeneratedContent}
              isGenerating={drafter.isGenerating}

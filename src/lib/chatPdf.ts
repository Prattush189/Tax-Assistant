...
/**
 * Render a markdown document to a branded PDF and trigger a download.
 * Runs entirely in the browser (jsPDF is lazy-imported so it stays out of
 * the main bundle).
 */
export async function downloadChatPdf(markdown: string, title: string): Promise<void> {
  ...
  // Header — brand + title + rule. Painted on page 1 and after each page
  // break; returns the Y at which body content should start.
  const paintHeader = (): number => {
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(15);
    doc.setTextColor(5, 150, 105); // emerald-600
    doc.text(brand, margin, margin);
  ...
  for (let p = 1; p <= total; p++) {
    ...
    doc.text(
      `Generated by ${brand} on ${stamp} - AI-generated, please verify with a qualified professional.`,
      margin,
      pageHeightMm - 8,
    );
